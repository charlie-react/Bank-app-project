...
let loanText = document.querySelector('.form__label--loan');
let monitor = document.querySelector('.eye');
let monitorpin = document.querySelector('.pin-eye');
let loginOut = document.querySelector('.login');
let logMeOut = document.querySelector('.log-me-out');
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    logMeOut.style.display = 'block';
    inputLoginUsername.value = '';
    inputLoginPin.value = '';
  } else if (!currentAccount) {
    monitor.textContent = 'user not found';
    monitor.style.color = `red`;
  } else {
    monitorpin.textContent = ` wrong pin!`;
    monitorpin.style.color = `red`;
  }
  // inputLoginPin.value = '';
  // inputLoginUsername.value = '';
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    ...
    loanText.textContent = `Amount`;
    loanText.style.backgroundColor = `transparent`;
    loanText.style.color = `black`;
  } else if (!currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    loanText.textContent = `invalid`;
    loanText.style.backgroundColor = `white`;
